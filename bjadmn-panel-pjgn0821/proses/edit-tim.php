...
// Ambil data
$stmt = $conn->prepare("SELECT * FROM karyawan WHERE id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error)) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if ($result['error']) {
            ...
            // Hapus foto lama
            if ($data['foto'] && file_exists(__DIR__ . '/../../assets/img/tim/' . $data['foto'])) {
                unlink(__DIR__ . '/../../assets/img/tim/' . $data['foto']);
            }
    ...
    if (empty($error)) {
        $stmt = $conn->prepare("UPDATE karyawan SET nama=?, jabatan=?, pengalaman=?, urutan=?, foto=? WHERE id=?");
        $stmt->bind_param("ssissi", $nama, $jabatan, $pengalaman, $urutan, $nama_file, $id);
        $stmt->execute();
    ...
    <div class="container-fluid px-3 px-md-4 py-4 d-flex flex-column align-items-center">
        ...
        <div class="form-card">
            <div class="form-card-title">
                <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Anggota Tim
            </div>
            ...
            <form method="post" action="edit-tim.php?id=<?php echo $data['id']; ?>" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select name="jabatan" class="form-select" required>
                        ...
                        <option value="<?php echo $j; ?>" <?php echo ($data['jabatan'] === $j) ? 'selected' : ''; ?>>
                            <?php echo $j; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="urutan" class="form-control" min="0"
                        value="<?php echo (int)$data['urutan']; ?>">
                ...
                <div class="mb-4">
                    ...
                    <?php if ($data['foto']): ?>
                    <img src="<?php echo '../../assets/img/tim/' . htmlspecialchars($data['foto']); ?>"
